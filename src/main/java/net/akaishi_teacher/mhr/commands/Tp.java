...
public class Tp extends MHRAbstractCommand {
	...
	public boolean execute(CommandSender sender, ArrayList<String> args) {
		...
		if (args.size() >= 3 && !field_00001) { //To player option
			p = pl.getServer().getPlayerExact(args.get(2));
			if (p != null) {
				loc = p.getLocation();
			} else {
		...
		pl.getHorsesControler().tp(num, loc, flag);
		...
		sender.sendMessage(replaceArgs(pl.getLang().getLocalizedString("Cmd_Out_Tp_Set"), replaceMap));
		return true;
	}

	@Override
	public String getUsage(CommandSender sender) {
		return pl.getLang().getLocalizedString("Cmd_Usage_Tp");
	}
